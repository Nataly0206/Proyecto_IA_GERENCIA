...
export async function fetchPeladoLive(refresh = false): Promise<PeladoLiveResponse> {
  const { data } = await apiClient.get<PeladoLiveResponse>('/dashboard/pelado-tiempo-real', {
    params: refresh ? { refresh: 'true' } : undefined,
  });
  return data;
}
